    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-amber-50 to-yellow-50">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <button
            onClick={onBack}
            className="flex items-center text-orange-600 hover:text-orange-700 transition-all duration-300 mb-6 font-semibold"
          >
          ...
          <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl p-8 border border-orange-100">
            <div className="flex items-start space-x-6">
              ...
              <div className="flex-1">
                <h1 className="text-3xl font-bold bg-gradient-to-r from-orange-600 to-amber-600 bg-clip-text text-transparent mb-2">{guide.title}</h1>
              ...
              <button
                ...
                className={`flex items-center space-x-2 px-4 py-2 rounded-xl font-medium transition-all duration-300 shadow-sm ${
                  selectedOS === os
                    ...
                    : 'bg-orange-50 text-gray-700 hover:bg-orange-100 hover:shadow-md'
                }`}
              >
                {getOSIcon(os)}
        ...
        <div className="space-y-6">
          <h2 className="text-2xl font-bold bg-gradient-to-r from-orange-600 to-amber-600 bg-clip-text text-transparent">Installation Steps</h2>

          {guide.steps[selectedOS]?.map((step, index) => (
            <div key={index} className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg border border-orange-100 overflow-hidden">
              <div className="p-6">
                <div className="flex items-start space-x-4">
                  ...
                  <div className="flex-1">
                      ...
                      {step.description.split(/(\*\*\[.*?\]\(.*?\)\*\*|"[^"]*")/).map((part, index) => {
                        ...
                        if (quoteMatch) {
                          return (
                            <strong
                              key={index}
                              className="bg-amber-100 px-1 rounded font-semibold text-gray-900"
                            >
                              {quoteMatch[1]}
                            </strong>
                          );
                    ...
                    {step.note && (
                      <div className="mt-4 p-4 bg-gradient-to-r from-amber-50 to-orange-50 border border-orange-200/50 rounded-xl shadow-sm">
        ...
        {guide.troubleshooting && guide.troubleshooting.length > 0 && (
          <div className="mt-12 bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg p-8 border border-orange-100">
            <h2 className="text-2xl font-bold bg-gradient-to-r from-orange-600 to-amber-600 bg-clip-text text-transparent mb-6">Common Issues & Solutions</h2>
            <div className="space-y-6">
              {guide.troubleshooting.map((item, index) => (
                <div key={index} className="border-l-4 border-orange-500 pl-6 bg-gradient-to-r from-orange-50/50 to-amber-50/50 py-3 rounded-r-xl">
